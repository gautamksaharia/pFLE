...
def compute_residuals(net, x, t):
    ...
    res_r = r_xt - r - (r**2 + m**2) * m_x + e * r_x
    res_m = m_xt - m + (r**2 + m**2) * r_x + e * m_x
...
def plot_outputs(net, x_range=(-L, L), t_range=(t0, tmax), resolution=100):
    ...
    axes[0].text(
    0.05, 0.99, "(c)",        # (x, y) in axes fraction coordinates
    ...
    #plt.savefig("soliton_snapshot_exp0_gaussian5.pdf", dpi=400, bbox_inches="tight")  # high-res save
    plt.show()
    ...
    plt.tight_layout()
    #plt.savefig("exp0_all_gaussian1.pdf", dpi=400, bbox_inches="tight")
    plt.show()
...
def spectral_domain1(psi):
    ...
    # === Optional in-panel label ===
    #ax.text(0.05, 0.92, "(b)", transform=ax.transAxes,
     #       fontsize=15, fontweight="bold", va="top", ha="left")

    # === Save and show ===
    plt.tight_layout()
    #plt.savefig("spectral_evolution_gaussian.pdf", dpi=400, bbox_inches="tight")
    plt.show()
...
np.save("exp0max_amplitude_e0.1_nonoise_1", max_ampl)
np.save("exp0enegy_e0.1_nonoise_1", energy_spatial)
np.save("exp0momentum_e0.1_nonoise_1", momentum_spatial)
#np.save("exp0_wave0.1_nonoise", u_pinn)
